...
{
  // START CODE HERE
  int val_fixed = val >> r_shift - ss_w_lg2;
  val_fixed = val_fixed << r_shift - ss_w_lg2;
  return val_fixed;
  // END CODE HERE
}
...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  // check backface
  bool if_backface_L = false;
  int backface = (triangle.v[1].x - triangle.v[0].x) * (triangle.v[2].y - triangle.v[0].y) - (triangle.v[2].x - triangle.v[0].x) * (triangle.v[1].y - triangle.v[0].y);
  if (backface < 0)
    if_backface_L = true;
  else
    if_backface_L = false;
  ...
  bbox.valid = (bbox.lower_left.x >= 0) && (bbox.lower_left.y >= 0)
  &&(bbox.upper_right.x <= screen.width) && (bbox.upper_right.y <= screen.height) && (if_backface_L);
...
bool sample_test(Triangle triangle, Sample sample)
{
  ...
  // get distance of origin shifted edge, for all three
  int dist_edge0 = v0_x * v1_y - v1_x * v0_y;
  int dist_edge1 = v1_x * v2_y - v2_x * v1_y;
  int dist_edge2 = v2_x * v0_y - v0_x * v2_y;

  // test if origin is on the right side of these edge
  bool b0 = dist_edge0 <= 0;
  bool b1 = dist_edge1 < 0;
  bool b2 = dist_edge2 <= 0;
